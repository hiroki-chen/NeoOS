...
// 为基于BIOS或UEFI提供的内存映射的物理帧分配器定义结构体
...
// 映射函数的实际实现。
///
/// 此函数将`virt_addr`开始的`size`内存字节映射到`phys_addr`，如果它不是
/// `None`，即，内存已分配；否则，它将在运行时明确分配一个物理帧。
pub fn map_addr(
...
/// 映射其余的空闲物理地址。
/// 注意这只是一个线性映射：0xffff_8880_0000_0000 + physical_address。
pub fn map_phy
pub fn map_physical(
    kernel: &Kernel,
